...
{
    public partial class LiBackgammonPropellerModule
    {
        private static readonly string[] _leftRight = ["left", "right"];
        private static readonly string[] _whiteBlack = ["white", "black"];
        ...
        private HttpResponse play(HttpRequest req)
        {
            ...
            var points =
                game.State is GameState.Black_Won_Finished or GameState.White_Won_Finished ? (pos.GameValue ?? 1) * pos.GetWinMultiplier(game.State == GameState.White_Won_Finished) :
                game.State is GameState.Black_Won_DeclinedDouble or GameState.White_Won_DeclinedDouble ? pos.GameValue ?? 1 :
                game.State is GameState.Black_Won_Resignation or GameState.White_Won_Resignation ? (pos.GameValue ?? 1) * pos.GetWinMultiplier(game.State == GameState.White_Won_Resignation) : 0;
            ...
            var activeGameSockets = GetSocketsByGame(publicId);
                ...
                new BODY(
                    new DIV
                    {
                        ...
                        class_ =
                            ...
                            + ((game.State is GameState.Black_ToMove or GameState.White_ToMove)
 ...
 + " debug"
                        ...
                        ._(
                            new DIV { id = "board" }._(
                                _leftRight.Select(loc => new DIV { class_ = "background main-area " + loc }),
                                _whiteBlack.Select(col => new DIV { class_ = "background home " + col }),
                                ...
                                _leftRight.Select(loc => new DIV { class_ = "shadow main-area " + loc }),
                                _whiteBlack.Select(col => new DIV { class_ = "shadow home " + col }),
                                ...
                                Enumerable.Range(0, 15).Select(pieceNum => _whiteBlack.Select(color => new DIV { class_ = "piece " + color })),
